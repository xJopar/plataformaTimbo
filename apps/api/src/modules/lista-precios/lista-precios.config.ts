export interface ZohoAnalyticsConfig {
  ...
  workspaceId: string;
  viewId: string;
  equipmentRentalViewId: string;
  clientId: string;
  clientSecret: string;
  refreshToken: string;
}

const DEFAULT_EQUIPMENT_RENTAL_VIEW_ID = '2400409000048195020';
...
export function resolveZohoAnalyticsConfig(
  ...
  return {
    ...
    equipmentRentalViewId: resolveRequiredZohoValue(
      env.ZOHO_EQUIPMENT_RENTAL_VIEW_ID ?? DEFAULT_EQUIPMENT_RENTAL_VIEW_ID,
      'ZOHO_EQUIPMENT_RENTAL_VIEW_ID',
    ),
