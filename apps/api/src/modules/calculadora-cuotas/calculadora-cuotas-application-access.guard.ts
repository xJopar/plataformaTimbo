...
const CALCULADORA_CUOTAS_APPLICATION_KEY = 'calculadora-cuotas';

@Injectable()
export class CalculadoraCuotasApplicationAccessGuard implements CanActivate {
  public constructor(
  ...
  public async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (authenticatedUser === undefined) {
      throw new Error(
        'El guard de sesión debe ejecutarse antes del guard de Calculadora de Cuotas.',
      );
    }

    const hasApplicationAccess = await this.applicationAuthorizationService.hasApplicationAccess(
      authenticatedUser.id,
      CALCULADORA_CUOTAS_APPLICATION_KEY,
    );
    if (!hasApplicationAccess) throw new AuthPublicError('AUTHORIZATION_REQUIRED', 403);
