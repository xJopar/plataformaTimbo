...
export class MetaCompanyController {
  ...
  public async updateBrandGoal(@Req() request: AuthenticatedRequest, @Param('id') id: string, @Body() body: Record<string, unknown>) { return mapBrandGoal(await this.metaCompanyService.updateBrandGoal(Number(id), stringValue(body.value), body.workingDays === undefined ? undefined : numberValue(body.workingDays), requireAuthenticatedUserId(request))); }
...
interface BrandGoalWithRelations { id: number; period: Date; businessId: number; brandId: number; value: { toFixed(digits: number): string }; workingDays: number | null; updatedAt: Date | null; business: { name: string }; brand: { name: string } }
