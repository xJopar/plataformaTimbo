...
export function PlatformHeader({
  ...
  const showBackBar = isApplicationHeader && breadcrumb !== undefined && onBack !== undefined;
  ...
  return (
    <>
        ...
        {isApplicationHeader ? (
          <div className="application-context">
            ...
            {breadcrumb === undefined ? null : (
              <p className="application-breadcrumb">{breadcrumb}</p>
            )}
          </div>
        ) : null}
              ...
              <nav className="application-switcher-menu" aria-label="Aplicaciones autorizadas">
                {applications.map((item) =>
                  item.launchPath === applicationLaunchPath ? (
                    <span
                      aria-current="page"
                      className="application-switcher-current"
                      key={item.key}
                    >
                      {item.name}
                    </span>
      ...
      {showBackBar ? (
        <nav className="app-breadcrumb-bar" aria-label="Ruta de navegación">
          <button
            ...
            onClick={onBack}
          >
            <AppIcon icon={ArrowLeft02Icon} size={20} strokeWidth={2.5} />
            {backLabel === undefined ? null : (
              <span className="app-breadcrumb-back-label">{backLabel}</span>
            )}
          </button>
        </nav>
      ) : null}
